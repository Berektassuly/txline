# Devnet Golden Fixtures

`validation_golden.devnet.json` and `trading_golden.devnet.json` store complete
Anchor instruction data bytes for the Devnet validation and low-level trading
builders. Rust tests read these checked-in files with `include_str!`, so the
normal Rust test suite does not require Node, Anchor, or a local
`txodds/tx-on-chain` checkout.
