...
export class FixturesClient {
  ...
  async validation(fixtureId: number, timestamp?: number): Promise<FixtureValidation> {
  ...
  async batchValidation(epochDay: number, hourOfDay: number): Promise<FixtureBatchValidation> {
    validateHour(hourOfDay);
...
export function validateInterval(interval: number): void {
  if (!Number.isInteger(interval) || interval < 0 || interval > 11) {
    throw new InvalidInputError("interval must be the 0-indexed 5-minute bucket 0..=11");
  }
}
